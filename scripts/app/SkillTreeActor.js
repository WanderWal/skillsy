...
export class SkillTreeActor extends HandlebarsApplication {
    ...
    activateContent(html) {
        ...
        const skillPointsInput = html.querySelector("input[name='skillPoints']");
        if (skillPointsInput && !skillPointsInput.hasAttribute("readonly")) {
            skillPointsInput.addEventListener("change", async (event) => {
                const value = parseInt(event.target.value);
                if (isNaN(value)) return;
                await setSkillPoints(this.actor, this.skillTree, value, { buildId: this.activeBuildId });
            });
        }
